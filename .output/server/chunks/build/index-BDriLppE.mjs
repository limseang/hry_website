      ...
      if (!unref(auth).isLoggedIn) {
        ...
        if (unref(loading)) {
          ...
          ssrRenderList(unref(orders), (o) => {
            var _a, _b;
            _push(`<div class="card p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5"><div class="flex-1 min-w-0"><div class="flex items-center gap-3 flex-wrap mb-2"><span class="font-mono font-500 text-sm text-cream-100">${ssrInterpolate(o.order_number)}</span><span class="${ssrRenderClass([statusClass(o.status), "badge text-[10px] uppercase tracking-wider"])}">${ssrInterpolate(o.status.replace(/_/g, " "))}</span></div><div class="font-body text-mist text-sm mb-1">${ssrInterpolate(((_a = o.items) == null ? void 0 : _a.length) || 0)} item${ssrInterpolate(((_b = o.items) == null ? void 0 : _b.length) !== 1 ? "s" : "")} `);
            if (o.customer_name) {
              _push(`<span> \xB7 ${ssrInterpolate(o.customer_name)}</span>`);
            } else {
              _push(`<!---->`);
            }
...
//# sourceMappingURL=index-BDriLppE.mjs.map
